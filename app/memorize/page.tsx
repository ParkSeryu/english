...
export default async function MemorizePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const nextExpression = displayQueue[0] ?? queue[0];
  const nextReturnTo = nextExpression && queue.length > 1 ? `/memorize?skip=${encodeURIComponent(nextExpression.id)}` : "/memorize";
  ...
  return (
    <div className="space-y-5">
      ...
      {nextExpression ? <><MemorizeCard expression={nextExpression} returnTo={nextReturnTo} /><p className="text-center text-sm font-semibold text-slate-500">오늘 암기 큐에 {queue.length}개 표현이 있습니다.</p></> : <EmptyState title="암기할 표현이 없습니다" body="LLM 초안을 승인하면 한국어 프롬프트로 외울 표현이 생깁니다." actionHref="/expressions" actionLabel="표현 보관함 보기" />}
    </div>
  );
}
