...
export default async function ExpressionsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div className="space-y-5">
      <div><p className="text-sm font-black uppercase tracking-[0.2em] text-teal-700">표현</p><h1 className="mt-2 text-3xl font-black text-ink">표현 모아보기</h1><p className="mt-3 text-sm leading-6 text-slate-600">수업에서 배운 표현을 날짜별로 다시 확인하세요.</p></div>
      {visibleDays.length === 0 ? <EmptyState title="아직 표현이 없습니다" body="배운 표현이 생기면 날짜별로 여기에 쌓입니다." actionHref="/memorize" actionLabel="암기 화면 보기" /> : (
        <div className="space-y-5">
          {visibleDays.map((day) => (
