...
export function PersonalExpressionForm({ targetExpressionDayId }: { targetExpressionDayId?: string | null }) {
  ...
  const [isMemorizationEnabled, setIsMemorizationEnabled] = useState(false);
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-card">
      ...
      <div>
        ...
        <textarea id="english" name="english" rows={3} value={english} onChange={(event) => setEnglish(event.target.value)} className="input min-h-24" placeholder="예: I'm trying to cut down on sugar." required />
        <FieldError messages={state.fieldErrors?.english} />
      </div>
      <div>
        <label className="label" htmlFor="koreanPrompt">한국어 뜻 / 암기 프롬프트</label>
        <textarea id="koreanPrompt" name="koreanPrompt" rows={3} value={koreanPrompt} onChange={(event) => setKoreanPrompt(event.target.value)} className="input min-h-24" placeholder="예: 설탕을 줄이려고 노력하고 있어요." required />
        <FieldError messages={state.fieldErrors?.koreanPrompt} />
      </div>
      <div>
        <label className="label" htmlFor="grammarNote">문법/패턴 메모</label>
        <textarea id="grammarNote" name="grammarNote" rows={3} value={grammarNote} onChange={(event) => setGrammarNote(event.target.value)} className="input min-h-24" placeholder="예: cut down on + 명사 = ~을 줄이다" />
